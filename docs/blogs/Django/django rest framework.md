...
![wallhaven-1kdzk3_1920x1080](/image/drf/wallhaven-1kdzk3_1920x1080.png)
...
![indepent_frontend_backend](/image/drf/indepent_frontend_backend.png)在前后端分离的应用模式中，后端仅返回前端所需的数据，不再渲染HTML页面，不再控制前端的效果。至于前端用户看到什么效果，从后端请求的数据如何加载到前端中，都由前端自己决定，网页有网页的处理方式，App有App的处理方式，但无论哪种前端，所需的数据基本相同，后端仅需开发一套逻辑对外提供数据即可。
...
![drf_logo](/image/drf/drf_logo.png)
...
![图书接口Web浏览页面](/image/drf/图书接口Web浏览页面.png)1)点击链接127.0.0.1:8000/books/ 可以访问**获取所有数据的接口**，呈现如下页面：

![查询所有图书信息1](/image/drf/查询所有图书信息1.png)

![查询所有图书页面2](/image/drf/查询所有图书页面2.png)

2）在页面底下表单部分填写图书信息，可以访问**添加新图书的接口**，保存新书：

![保存新图书信息](/image/drf/保存新图书信息.png)

点击POST后，返回如下页面信息：

![保存图书返回信息](/image/drf/保存图书返回信息.png)

3）在浏览器中输入网址127.0.0.1:8000/books/1/，可以访问**获取单一图书信息的接口**（id为1的图书），呈现如下页面：

![获取单一图书信息](/image/drf/获取单一图书信息.png)

4）在页面底部表单中填写图书信息，可以访问**修改图书的接口**：

![修改图书信息](/image/drf/修改图书信息.png)

点击PUT，返回如下页面信息：

![修改图书返回信息](/image/drf/修改图书返回信息.png)

5）点击DELETE按钮，可以访问**删除图书的接口**：

![删除图书](/image/drf/删除图书.png)

返回，如下页面：

![删除返回信息](/image/drf/删除返回信息.png)
...
](../C03-Serializer/Serializing.html)
...
![通用视图的继承关系](/image/drf/通用视图的继承关系.png)

视图的方法与属性：

![通用视图的继承关系_含方法和属性](/image/drf/通用视图的继承关系_含方法和属性.png)
...
![视图集类继承关系](/image/drf/视图集类继承关系.png)
...
![simple_router](/image/drf/simple_router.png)

2）DefaultRouter

![default_router](/image/drf/default_router.png)
